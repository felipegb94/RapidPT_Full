...
function [ outputs ] = OneSampleRapidPermutationTesting( inputs )
...
%   Speeding up Permutation Testing in Neuroimaging  C Hinrichs, VK Ithapu, Q Sun, SC Johnson, V Singh, NIPS 2013
    ...
    numPermutations = size(permutationMatrix,1);
...
%% Training for low rank subsapace and residual priors
    ...
    permutationMatrixCurrent = permutationMatrix(1:1:trainNum,:);

    
    tTrainPermTestIterative = tic;
    % Calculate some full permutations for training U (A good number would be the number of labels).
    [maxTDummy, TCurrent] = OneSamplePermTest(data, dataSquared, permutationMatrixCurrent, maxMemory);
...
%
    ...
    t = 1;
    % Calculate 0.5% of the permutation matrix
    % TCurrentSmall = zeros(numPermutations,subV); 
    for i = 1:numPermutations
        r = randperm(V); 
        inds = r(1:subV)';
        tPermTest = tic;
        [maxTDummy,TCurrent] = OneSamplePermTest(data(:,inds),...
                                     dataSquared(:,inds),...
                                     permutationMatrix(i,:),...
                                     maxMemory);
                              
        tAllPermTest(1, counter) = toc(tPermTest);
        U_inds = UHat(inds,:);  
        tSrp = tic;
    ...
    description = strcat('_',num2str(numPermutations),'_',num2str(sub),'_',num2str(trainNum));
    outputFilename = strcat('outputs', description);
